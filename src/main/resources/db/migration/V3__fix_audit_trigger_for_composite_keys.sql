-- ============================================
-- Migration: Fix audit trigger for composite keys
-- Description: Update audit function to handle tables without 'id' column
-- ============================================

-- Drop existing audit trigger function
DROP FUNCTION IF EXISTS audit.log_audit_event() CASCADE;

-- Recreate audit trigger function with composite key support
...
BEGIN
    ...
    IF TG_OP = 'DELETE' THEN
        ...
        -- Try to get id, use 0 if not available (for join tables)
        BEGIN
