...
public interface PaymentMethodJpaRepository extends JpaRepository<PaymentMethodEntity, Long> {
     ...
     */
    @Query("SELECT p FROM PaymentMethodEntity p WHERE p.userId = :userId AND p.isActive = true ORDER BY p.isDefault DESC, p.createdAt DESC")
     ...
     */
    @Query("SELECT p FROM PaymentMethodEntity p WHERE p.userId = :userId AND p.isDefault = true AND p.isActive = true")
     ...
     */
    @Query("SELECT p FROM PaymentMethodEntity p WHERE p.cardExpYear = :year AND p.cardExpMonth = :month AND p.isActive = true")
     ...
     */
    @Query("SELECT CASE WHEN COUNT(p) > 0 THEN true ELSE false END FROM PaymentMethodEntity p WHERE p.userId = :userId AND p.isActive = true")
     ...
     */
    @Query("SELECT COUNT(p) FROM PaymentMethodEntity p WHERE p.userId = :userId AND p.isActive = true")
