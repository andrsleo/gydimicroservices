...
@Service
public class UnblockDatesUseCaseImpl implements UnblockDatesUseCase {

    private final PropertyRepositoryPort propertyRepository;
    private final CalendarDayRepositoryPort calendarDayRepository;

    public UnblockDatesUseCaseImpl(
    ...
    public void execute(Long propertyId, LocalDate date, Long requestingHostId) {
        ...
        CalendarDay calendarDay = calendarDayRepository.findByPropertyIdAndDate(propertyId, date)
                .orElseThrow(() -> new DateNotBlockedException(date, propertyId));

        // unblock() throws CannotUnblockReservedDateException if RESERVED
        CalendarDay unblockedDay = calendarDay.unblock();

        if (unblockedDay.getStatus() == CalendarDayStatus.AVAILABLE && !unblockedDay.hasCustomPrice()) {
            calendarDayRepository.delete(propertyId, date);
        } else {
            calendarDayRepository.save(unblockedDay);
        }
    }
}
