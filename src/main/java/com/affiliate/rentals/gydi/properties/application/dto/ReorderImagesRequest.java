...
public record ReorderImagesRequest(
                @NotNull(message = "Image orders cannot be null") @NotEmpty(message = "At least one image order is required") @Valid List<ImageOrderItem> imageOrders,
        ...
        public record ImageOrderItem(
                        ...
                        @NotNull(message = "Display order cannot be null") @Min(value = 0, message = "Display order must be >= 0") @Max(value = 19, message = "Display order must be <= 19") Integer displayOrder) {
        }
}