 ...
 * </p>
 * <ul>
 * <li>token: Must not be blank (Long format)</li>
 * <li>newPassword: Must be between 8-100 characters</li>
 * <li>newPassword: Must contain at least one uppercase, one lowercase, one
...
public record ResetPasswordRequest(
                ...
                @NotBlank(message = "New password is required") @Size(min = 8, max = 100, message = "Password must be between 8 and 100 characters") @Pattern(regexp = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]+$", message = "Password must contain at least one uppercase letter, one lowercase letter, one number, and one special character (@$!%*?&)") String newPassword) {
}
